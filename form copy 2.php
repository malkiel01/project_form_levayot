...
<body>
    ...
    <script>
        ...
        // הוסף את הקוד הזה לסוף הסקריפט ב-form.php
        ...
        function autoSave() {
            ...
            $.ajax({
                ...
                error: function() {
                    console.error('שגיאה בשמירה אוטומטית');
                },
                complete: function() {
                    isSaving = false;
        ...
        function updateFormStatus(status, progress) {
            ...
            $('.progress-bar').css('width', progress + '%').text(progress + '% הושלם');
            
            // עדכן את תג הסטטוס
            const statusIndicator = $('.status-indicator');
            if (statusIndicator.length) {
                if (status === 'completed') {
                    statusIndicator.removeClass('status-draft').addClass('status-completed').text('הושלם');
                } else {
                    statusIndicator.removeClass('status-completed').addClass('status-draft').text('טיוטה');
                }
            }
        }

        // פונקציה להצגת הודעת שמירה
        ...
        $('#deceasedForm').on('submit', function() {
