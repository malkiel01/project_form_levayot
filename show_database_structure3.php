...
<body>
    <div class="container mt-4">
        ...
        <ul class="nav nav-tabs" role="tablist">
            ...
            <li class="nav-item">
                <a class="nav-link" data-bs-toggle="tab" href="#backup">גיבוי ושחזור</a>
            </li>
        </ul>
            ...
            <div id="backup" class="tab-pane fade">
                <div class="table-container">
                    ...
                    <div class="alert alert-info">
                        <strong>חשוב!</strong> בצע גיבוי מלא לפני כל שינוי משמעותי במבנה מסד הנתונים
                    </div>
                        ...
                        <div class="col-md-4">
                            <div class="card">
                                <div class="card-header bg-primary text-white">
                                    <h5 class="mb-0">גיבוי מבנה בלבד</h5>
                                </div>
                                <div class="card-body">
                                    <p>מייצא רק את מבנה הטבלאות ללא נתונים</p>
                                    <form method="post" action="export_structure.php">
                                        <input type="hidden" name="export_type" value="structure">
                                        <button type="submit" class="btn btn-primary w-100">
                                            <i class="bi bi-download"></i> הורד מבנה
                                        </button>
                                    </form>
                                    <small class="text-muted">כולל: CREATE TABLE, אינדקסים, FK</small>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="card">
                                <div class="card-header bg-success text-white">
                                    <h5 class="mb-0">גיבוי נתונים בלבד</h5>
                                </div>
                                <div class="card-body">
                                    <p>מייצא רק את הנתונים ללא מבנה</p>
                                    <form method="post" action="export_data.php">
                                        <input type="hidden" name="export_type" value="data">
                                        <button type="submit" class="btn btn-success w-100">
                                            <i class="bi bi-database"></i> הורד נתונים
                                        </button>
                                    </form>
                                    <small class="text-muted">כולל: כל הנתונים בפורמט INSERT</small>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="card">
                                <div class="card-header bg-warning text-dark">
                                    <h5 class="mb-0">גיבוי מלא</h5>
                                </div>
                                <div class="card-body">
                                    <p>מייצא מבנה + נתונים</p>
                                    <form method="post" action="export_full.php">
                                        <input type="hidden" name="export_type" value="full">
                                        <button type="submit" class="btn btn-warning w-100">
                                            <i class="bi bi-archive"></i> הורד גיבוי מלא
                                        </button>
                                    </form>
                                    <small class="text-muted">כולל: הכל - מבנה ונתונים</small>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- גיבוי לפי טבלאות -->
                    <div class="mt-4">
                        <h5>גיבוי סלקטיבי - בחר טבלאות</h5>
                        <form id="selectiveBackupForm" method="post" action="export_selective.php">
                            <div class="row">
                                <div class="col-md-6">
                                    <h6>בחר טבלאות לגיבוי:</h6>
                                    <div style="max-height: 300px; overflow-y: auto; border: 1px solid #ddd; padding: 10px;">
                                        <?php
                                        foreach ($tables as $table) {
                                            $rowCount = $db->query("SELECT COUNT(*) FROM `$table`")->fetchColumn();
                                            echo "<div class='form-check'>";
                                            echo "<input class='form-check-input table-checkbox' type='checkbox' name='tables[]' value='$table' id='chk_$table'>";
                                            echo "<label class='form-check-label' for='chk_$table'>";
                                            echo "$table <small class='text-muted'>($rowCount רשומות)</small>";
                                            echo "</label>";
                                            echo "</div>";
                                        }
                                        ?>
                                    ...
                                    <div class="mt-2">
                                        <button type="button" class="btn btn-sm btn-outline-primary" onclick="selectAllTables()">בחר הכל</button>
                                        <button type="button" class="btn btn-sm btn-outline-secondary" onclick="deselectAllTables()">בטל הכל</button>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <h6>אפשרויות גיבוי:</h6>
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="include_structure" id="include_structure" checked>
                                    ...
                                    <div class="form-check">
                                        ...
                                        <label class="form-check-label" for="drop_tables">
                                            הוסף DROP TABLE לפני CREATE
                                        </label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="disable_fk" id="disable_fk" checked>
                                        <label class="form-check-label" for="disable_fk">
                                            בטל Foreign Keys בזמן השחזור
                                        </label>
                                    </div>
                                    <hr>
                                    <button type="submit" class="btn btn-primary w-100">
                                        <i class="bi bi-download"></i> הורד גיבוי סלקטיבי
                                    </button>
                                </div>
                            </div>
                        </form>
                    ...
                    <div class="mt-4">
                        ...
                        <div class="alert alert-secondary">
                            <ol>
                                <li>הורד את קבצי הגיבוי למחשב</li>
                                <li>פתח את phpMyAdmin או כלי MySQL אחר</li>
                                <li>בחר את מסד הנתונים הרצוי</li>
                                <li>לחץ על "Import" או "ייבוא"</li>
                                <li>בחר את קובץ ה-SQL שהורדת</li>
                                <li>לחץ על "Go" או "בצע"</li>
                            </ol>
                            <strong>טיפ:</strong> לגיבויים גדולים, השתמש בשורת הפקודה:
                            <pre>mysql -u username -p database_name < backup_file.sql</pre>
                        </div>
                    </div>
                    
                    <!-- סטטיסטיקות גיבוי -->
                    <div class="mt-4">
                        <h5>סטטיסטיקות מסד נתונים</h5>
                        <?php
                        // חישוב גודל כולל
                        $dbSize = $db->query("
                            SELECT 
                        ...
                        $totalRows = 0;
                        foreach ($tables as $table) {
                            $totalRows += $db->query("SELECT COUNT(*) FROM `$table`")->fetchColumn();
                        }
                        ?>
                        <div class="row">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3">
                                <div class="card text-center">
                                    <div class="card-body">
                                        <h4><?php echo number_format($totalRows); ?></h4>
                                        <p class="mb-0">רשומות</p>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="card text-center">
                                    <div class="card-body">
                                        <h4><?php echo number_format($dbSize, 2); ?> MB</h4>
                                        <p class="mb-0">גודל</p>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="card text-center">
                                    <div class="card-body">
                                        <h4><?php echo $fkCount; ?></h4>
                                        <p class="mb-0">קשרי FK</p>
                                    </div>
                                </div>
                            </div>
                        </div>
